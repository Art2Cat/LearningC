...
int main(void) {
  ...
  assert(strcmp(buf, " a|-4  |0010|OXC") == 0);
  assert(sscanf(buf, " %c| %hd | %i |%lx", &ch, &sh, &in, &lo) == 4);
  assert(ch == 'a' && sh == -4 && in == 8 && lo == 12);
  assert(sprintf(buf, "%E|%.2f|%Lg", 1.1e20, -3.346, 0.02L) == 23);
  ...
  assert(fabs(fl - 0.02) / 0.02 < 4 * LDBL_EPSILON);
  assert(4 <= sprintf(buf, "|%%%n %p", &in, (void *)&ch) && in == 2);
...
void testFormattedIO() {
  ...
  assert(fclose(pf) == 0 && freopen(tn, "r", stdin) == stdin);
