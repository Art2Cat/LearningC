...
int main(void) {
  ...
  assert(memcmp((char *)memmove(s + 2, s, 3) - 2, "bcece", 6));
  ...
  assert(strlen("") == 0);
  assert(strncat(strcpy(s, abcde), "fg", 1) == s && strcmp(s, "abcdef") == 0);
  assert(strncmp(abcde, "abcde", 30) == 0);
  assert(strncmp(abcde, abcdx, 30) != 0);
  assert(strncmp(abcde, abcdx, 4) == 0);
  assert(strncpy(s, abcde, 7) == s && memcmp(s, "abcde\0", 7) == 0);
  assert(strncpy(s, "xyz", 2) == s && strcmp(s, "xycde") == 0);
  ...
  assert(strtok(NULL, "ace") == NULL && memcmp(s, "ab\0d\0\0", 6) == 0);
  n = strxfrm(NULL, abcde, 0);
  if (n < sizeof(s) - 1) {
    assert(strxfrm(s, abcde, n + 1) == n && strlen(s) == n);
  }
